...
--
-- NOTE: `[i` and `[d` are not the built-in include/define search (`:h [i`,
-- `:h [d`). Neovim's defaults take `[d` for LSP (`:h lsp-defaults`) and
-- 'mini.indentscope' takes `[i` further down this file, so the `indent` target
-- is disabled below in its favor (`:h MiniBracketed.config`).
later(function()
  require('mini.bracketed').setup({
    -- 'mini.indentscope' maps `[i` / `]i` later in this file
    indent = { suffix = '' },
  })
end)
