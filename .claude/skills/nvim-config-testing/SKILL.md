...
| `:normal <Space>ei` non preme il Leader: `:h :normal` mangia lo spazio iniziale dell'argomento | manda i tasti con `feedkeys` — cioè con `press` della sonda `keymap`, che usa `P.keys()` |
