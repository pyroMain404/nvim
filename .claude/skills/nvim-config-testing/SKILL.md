...
| `cat -A` e `grep` di Git Bash **tolgono il CR** di un file CRLF: la riga finisce con `$` invece che con `^M$`, il file sembra a LF e da lì lo si riscrive a LF, che è l'antipattern del line ending | non guardare i fine riga, contali: `file` risponde `with CRLF line terminators`, `tr -cd` dei due caratteri dà i due totali (uguali se il file è coerente), e il conteggio va rifatto **dopo** aver scritto |
