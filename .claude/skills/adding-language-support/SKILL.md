...
1. **Treesitter**: se il linguaggio non ha già un parser, aggiungilo a `ensure_installed` in `lua/user/treesitter.lua`. Verifica con `+TSInstallSync <lingua>` e controlla il `.so` risultante — l'output async di `TSUpdate` mente ("up-to-date" senza che il parser esista): fidati del `.so` in `nvim-data\lazy\nvim-treesitter\parser\`, non del messaggio.
