...
- Load `Shatur/neovim-ayu`'s `ayu-mirage` variant (`:colorscheme ayu-mirage`),
  instead of `purplehue`, this config's own scheme generated by `mini.colors`.
  `colors/purplehue.lua` is deleted along with every reference to it in
  `AGENTS.md`; `ayu-mirage` needs no regeneration step when a color changes.

- Ponytail-audit cleanup pass across `configs/nvim-0.12`: collapsed the six
  hand-written async Git runners and four duplicated commit pickers in
  `plugin/41_git.lua`/`plugin/43_review.lua` into `Config.git.run`/`.fail`/
  `.pick_commit`; removed a speculative Ant `build.xml` resolver from
  `lua/config/run.lua` (no Ant project exists in either workspace this config
  serves) and the matching dead wiring in `after/ftplugin/java.lua` and
  `xml.lua`; removed a 25-line, 2-blocking-call rediscovery of a Java version
  minimum in `lua/config/health.lua` that was already hardcoded as its own
  fallback; deleted several commented-out design essays (an unimplemented
  `'winbar'`, an unimplemented async `:make`, a Termdebug tutorial) in favor of
  one-line `TODO`/`NOTE` pointers; shrunk keymap-group comments that restated
  `plugin/41_git.lua`/`plugin/43_review.lua`'s own file headers. No mapping,
  `:Run`/`:make` contract, or user-visible behavior changed except the
  colorscheme swap above. Findings and rationale recorded in
  `docs/ENGINE_AUDIT.md` (P1-P26).

- Use PowerShell 7 (`pwsh`), not the Windows default `cmd.exe`, as Neovim's
  `'shell'` for `:!`, `:make`, `system()` and the terminal. Fixes
  `v:shell_error` being unusable after `:make`: `cmd.exe`'s
  `2>&1| tee %s` always reported `tee`'s own exit code (always 0), while
  `pwsh`'s `$LastExitCode` correctly survives `Tee-Object`.

- Implement the config's own three remaining actionable `TODO`s: `:GodotTree`
  in the new `after/ftplugin/gdresource.lua` renders a `.tscn`/`.tres`'s node
  tree as a scratch buffer (parsed with the `godot_resource` tree-sitter
  grammar), with `<CR>` jumping to a node's section in the source file;
  `'winbar'` now shows a tree-sitter container breadcrumb (function/class/
  struct/etc.) under the cursor, cached on `CursorMoved` rather than
  recomputed every redraw; `:Make`/`:LMake` in `plugin/10_options.lua` are
  async, `vim.system()`-backed equivalents of `:make`/`:lmake`, reusing the
  existing `QuickFixCmdPost` report; `:HipatternsLoc`/`:HipatternsGrep`
  (`<Leader>ft`/`<Leader>fT`) collect the same `FIXME`/`HACK`/`TODO`/`NOTE`
  markers `mini.hipatterns` highlights into a location list (current buffer)
  or quickfix list (whole project via `rg`), built from one shared word table
  so the highlighters and the search pattern cannot drift apart. The three
  remaining `TODO`s (Godot test/export tasks, a Gradle `:compiler` plugin, a
  GDExtension build-system health advisory) stay as-is: each names a real
  project on this machine as its own first step, and none exists yet.
