...
-- `:checkhealth user.floatapps` — riporta per ogni app della lista in init.lua se
-- l'eseguibile e' nel PATH (voce attiva) o manca (voce inattiva, silently-continue).
function M.check()
  vim.health.start "user.floatapps — TUI in terminale flottante (<leader>o)"

  local apps = require("user.floatapps").apps
