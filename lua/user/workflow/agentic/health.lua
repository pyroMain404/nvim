-- :checkhealth user.workflow.agentic — dipendenze ESTERNE del plugin agentic.nvim.
-- Filosofia: SEGNALARE, non silenziare (come user.node/user.workflow.jobs). Il plugin NON
...
function M.check()
  local h = vim.health
  h.start "user.workflow.agentic (agenti AI via ACP)"
  ...
  -- 2. Provider ACP configurato (letto dalla spec user.workflow.agentic, non hardcodato qui).
  local provider = "claude-agent-acp"
  local spec_ok, spec = pcall(require, "user.workflow.agentic")
