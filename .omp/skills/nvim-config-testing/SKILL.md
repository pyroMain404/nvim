...
| `hub send` con `text` che contiene `<CR>` letterale (`"gg0/label<CR>"`) lo **digita come quattro caratteri**, non lo traduce in Enter: la ricerca fallisce con `E486: Pattern not found: label<CR>` e sembra un problema del pattern | manda il testo senza `<CR>` e lascia l'`enter` di default (`true`), oppure `keys: ["ENTER"]` a parte. `<CR>`/`<Esc>`/notazioni Vim non sono interpretate da `text`, solo da `keys` |
| Incatenare più `hub send` in una sessione PTY senza controllare lo stato fra un invio e l'altro: un tasto che doveva completare una combinazione (`<Leader>oG`) può arrivare mentre Neovim è ancora altrove (un comando precedente non ancora tornato a Normal), e i tasti successivi vengono **digitati come testo dentro il buffer** invece di eseguire il mapping — nessun errore, il buffer si corrompe in silenzio (misurato: `"oG:lua ..."` finito dentro il file) | dopo un comando che potrebbe non essere tornato subito a Normal, verificare con `hub logs` (o uno `:lua` che scrive un marcatore) prima di mandare la combinazione successiva; se il buffer non è mai stato salvato il file su disco resta comunque intatto — controllarlo prima di continuare |
...
| Le heredoc di Bash su questa macchina collassano `\\` in `\`, e un `'\''` dentro una stringa Python la tronca. Non sono solo le heredoc: anche in un argomento fra apici singoli, un `\\r\\n` scritto per ottenere la sequenza letterale arriva a `perl -i -pe` come `\r\n`, e quello che finisce nel file è un vero CRLF | non generare file con backslash da script (usa `vim.fs.dirname`, `[char]92`), e non cercarne: un blocco `old` che contiene un escape ottale (la NOTE su `core.quotePath` in 'plugin/43_review.lua') arriva a Python già risolto in un carattere accentato, l'`assert` fallisce e la riga sembra identica a quella nel file. Spezza la sostituzione attorno alla riga con il backslash, e rileggi sempre la riga scritta. Il CRLF scritto per sbaglio così non lo vede nessun controllo sui fine riga: `git ls-files --eol` resta `w/crlf` e il conteggio torna, perché il file è tutto CRLF con una riga in più - e in una tabella Markdown quella riga ha appena smesso di essere una riga della tabella. Quando il testo da scrivere contiene un backslash, passa dallo strumento di edit invece che dalla shell. Un escape corrotto **dentro un commento** non fa fallire nessuna sonda: dopo un'edit scriptata la verifica è `git diff` letto a occhio, non la passata headless |
...
| Una notifica letta **nello stesso tick** in cui è stata emessa non c'è ancora: `vim.notify()` la registra attraverso uno `schedule`, quindi uno snippet che notifica e poi legge `MiniNotify.get_all()` conta solo quelle di prima — e un comando che rifiuta *dicendolo* sembra rifiutare in silenzio | lascia girare lo schedule prima di leggere (`vim.wait(50, function() return false end)`). Misurato: senza attesa la nota del rifiuto non compare né in `get_all()` né in `:messages`, con l'attesa compaiono entrambe insieme a quelle emesse dalla sonda stessa |
...
| I parametri di ricerca delle sonde (`expect`, `absent`, `pattern`, `allow`) sono **pattern Lua**, non regex: sono passati a `string.find()`. `C\+\+17` non trova `a C++17 extension` — in un pattern Lua l'escape è `%`, quindi `\+` significa "un backslash seguito da un carattere qualsiasi" — e `\.` in `compiler[\\/]cmake\.lua` non trova `compiler\cmake.lua`. Il rapporto è un `FAIL` con accanto l'evidenza che contiene proprio la stringa cercata: si legge come un guasto della config, ed è la sonda a chiedere la cosa sbagliata | scrivi `C%+%+17`, `compiler.cmake%.lua`, `^path=%.,,`. Il segnale che accusa la chiamata invece della config è l'evidenza stampata sotto il `FAIL`: se contiene il testo che il pattern doveva trovare, il pattern è sbagliato |
| Un linguaggio appena aggiunto alla tabella `languages` non ha ancora il parser, e il primo file di quel tipo aperto in headless **lancia**: l'autocomando `FileType` chiama `vim.treesitter.start()`, che asserisce, e la sonda `startup` riporta `E5108` con un traceback che punta a 'plugin/40_plugins.lua'. Accusa la modifica appena fatta, mentre manca solo un `.so` | installa i parser **prima** della passata, in modo sincrono e con un file argomento (senza argomento 'nvim-treesitter' non viene nemmeno aggiunto: è in `now_if_args`): `nvim --headless <file>.lua -S install.lua` con dentro `require('nvim-treesitter').install({...}, { force = true }):wait(900000)`, e conferma contando i `parser/<lang>.*` su 'runtimepath'. Serve un compilatore C sul `PATH` del processo, non solo sulla macchina |
| In `eval` JavaScript su Windows le chiavi di `process.env` conservano la maiuscola di Windows: **`Path`, non `PATH`**. Uno spread con `{ ...process.env, PATH: '...' }` aggiunge una *seconda* variabile che il creatore di processi ignora, e il figlio gira con il `PATH` originale: il sintomo è un programma appena installato che risulta non trovato (`clang++ is not a full path and was not found in the PATH` da CMake) | trova la chiave prima di scriverla (`Object.keys(env).find(k => k.toLowerCase() === 'path')`), oppure passa l'ambiente da `pwsh` con `$env:PATH = ... + $env:PATH`, che è insensibile al caso |
...
| La sonda `command` con `run` **esegue davvero** ciò che il comando fa, e per un comando che avvia un'applicazione (`:Run` di 'after/ftplugin/java.lua') quello che partirebbe è l'applicazione, dentro un Neovim che il watchdog uccide poco dopo | verifica **quale** comando sarebbe partito, non l'applicazione: nello `before` sostituisci `vim.fn.jobstart` (o `vim.system`) con uno stub che ne registra gli argomenti in una `vim.g.<nome>`, e leggila in `after`. L'assegnazione a `vim.fn.<nome>` funziona — verificato con `-u NONE` — ed è ciò che rende falsificabile la scelta del comando: tre fixture, tre `PROBE_CMD` diversi. Vale anche per un `:make` che gira a lungo |
...
| Un `BufReadCmd` che carica un altro file non fa scattare **niente** sul buffer che carica, se l'autocomando non è `nested`: gli autocomandi non si innescano a vicenda (`:h autocmd-nested`), quindi non c'è rilevamento del filetype, non c'è `FileType`, non c'è client LSP e non c'è ftplugin. La sonda legge `filetype` vuoto e zero client, e accusa il rilevamento del filetype, che è a posto (misurato su `res://`, che è esattamente questa forma) | isola il meccanismo in un Neovim a sé, con lo stesso autocomando registrato `nested = false` e `nested = true`: misurato `filetype=""` e nessun evento, contro `gdscript` con `BufReadPost` e `FileType`. Nell'autocomando vero il sospetto è il `:edit` che esegue, non chi lo riceve |
| Il cursore con cui un buffer si apre **non è `(1, 0)`**: `MiniMisc.setup_restore_cursor()` lo riporta all'ultima posizione registrata, su `FileType` (misurato: `{7, 27}` con la config contro `{1, 0}` con `--clean`; `{5, 4}` su un file appena aperto da `gf`). Uno snippet `before` che cerca dal cursore non è quindi deterministico, e su una modifica che fa scattare `FileType` cambia esito fra baseline e fix: la sonda accusa la config, e il rapporto sembra dire che `gf` risolve il riferimento sbagliato | fissa la posizione con `vim.fn.cursor(N, 1)` e cerca con il flag `'W'`. La fixture di `res://` ha **due** riferimenti nel file: senza fissare la riga la sonda risponde sul secondo, che è un `res://ran.txt` **inesistente**, e il `filetype` che ne esce (`text`, da un nome che non c'è su disco) sembra un guasto della risoluzione |

| Misurare **da una shell** se Neovim si blocca su `:qa` con un processo staccato risponde sempre di sì, e la colpa sembra della config: su Windows `CreateProcess` passa al figlio *tutti* gli handle ereditabili, quindi anche il nipote tiene aperto lo stdout rediretto e la pipeline (o `Start-Process -RedirectStandardOutput`) torna solo quando quello muore. Misurato su un gioco da 20 s: la shell è tornata a 21 s mentre Neovim se n'era andato a 3,05 s. E `handle:unref()` non è la spiegazione: dopo la chiamata `is_active` resta `true` e l'attesa è identica | misura da **dentro** Neovim, con un autocomando `VimLeavePre` che scrive `vim.uv.now()`: quel valore dice quando l'editor è uscito, mentre il ritorno della shell dice quando è morto il processo figlio. Vale per ogni controllo su un processo che sopravvive a Neovim |
| La callback `on_exit` di `vim.system()` gira in un **fast event context**: `vim.fn.writefile` dentro di essa lancia `E5560: Vimscript function "writefile" must not be called in a fast event context`, e il traceback compare mentre la sonda sta girando — identico a un guasto della config | avvolgi il corpo in `vim.schedule()`, e vale anche per `vim.notify()`. Un accumulatore di stringhe aggiornato *dentro* la callback funziona comunque: sono le API di Vimscript a essere vietate, non lo stato Lua |
| Uno snippet `before` che registra un esito con `vim.g.notes = {}` e poi `table.insert(vim.g.notes, …)` non registra niente: leggere `vim.g` restituisce una copia, quindi `after` rilegge la lista di partenza, vuota, e il rapporto mostra `NOTES: {}` per un controllo che invece ha funzionato | tieni il raccoglitore in una **globale Lua** (`probe_notes` in `_G`), o scrivi un file dalla callback. `vim.g` va bene per un valore scalare riletto dopo, non per mutare una tabella |
| Una sessione PTY (`hub start`) può essere già in **Insert** quando arriva il primo testo: i tasti finiscono nel buffer invece di eseguire il comando, il file risulta `[+]` modificato e il rapporto non prova niente (misurato: `:Run --headless` dentro 'main.gd'). La modalità sta nella status line, quindi `hub logs` la dice prima di scrivere | per una verifica di superficie non serve digitare: passa il comando a `-c` all'avvio (`nvim file.gd -c 'Run --headless'`) e leggi lo schermo con `hub logs`. Se la sessione va uccisa con il buffer modificato, cancella lo swap che lascia, o la successiva si ferma su `E325` senza eseguire niente |
