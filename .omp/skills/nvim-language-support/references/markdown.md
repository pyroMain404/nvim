...
`vim.pack` fetches it and maintains `nvim-pack-lock.json`. `vim.pack.add()` makes
the plugin usable before the following Lua statement; a missing module therefore
